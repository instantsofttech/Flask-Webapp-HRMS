...
def reg():
    ...
    if request.method=='POST':
        ...
        cur.execute("Insert into users(UNAME,UPASS,EMAIL,PICTURE,DEPARTMENT,FILEPATH,CONTACT,EMERGENCY) values(%s,%s,%s,%s,%s,%s,%s,%s)",(name,pwd,email,f1,dep,file_path,con,emergency))
...
def upload():
    ...
    for row in csv_data:
        conn.execute("INSERT INTO db_sample.users (UNAME, EMAIL, CONTACT, DEPARTMENT, EMERGENCY) VALUES (%s,%s,%s,%s,%s)", row)
        print(row)
        mysql.connection.commit()
        print("done!")
    conn.close()    
